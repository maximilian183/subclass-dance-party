var makeStarTrekDancer = function(top, left, timeBetweenSteps) {
  ...
  if (window.CROWNEDKING) {
    alert('Victory is decided!  No one else wants to joing the party.')
    return false;
  }
...
makeStarTrekDancer.prototype.attack = function(node) {

  var targetNode = this.findClosestTarget(node);
  //Finding closest node to the node you clicked on

  $(targetNode).addClass('target');

  //Creating a monkey node
  this.$node = $('<span class="monkeyDancer" style="top: ' + $(node).css('top') + '; left: ' + $(node).css('left') + '"><img src="img/monkey.png" height="100" width="100"></span>');
  ...
  var pathx = parseInt($(targetNode).css('left')) - parseInt($(node).css('left'));
  var pathy = parseInt($(targetNode).css('top')) - parseInt($(node).css('top'));
  ...
  if (window.dancers.length === 1 && !window.CROWNEDKING){
    this.lineUp($(node));
    var type = $(node).attr('class').replace('starTrekDancer ', '');
    type = type.charAt(0).toUpperCase() + type.slice(1);
    var victory = '<h1 class="victor">' + type + ' Victory</h1>'
    $('body').append(victory);
    window.CROWNEDKING = true;

    $(node).children().eq(0).css('transform', 'scale(3)');
    $(node).children().eq(0).css('-webkit-transform', 'scale(3)');
  }
};

makeStarTrekDancer.prototype.findClosestTarget = function(node){
